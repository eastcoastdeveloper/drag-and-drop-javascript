...
export class AppComponent {
  ...
  collectDropZones() {
    this.directoryElements = Array.from(document.querySelectorAll(".child-container .drop-zone"));
  ...
  deleteItem(i: number) {
    ...
    if(cContainer.innerHTML != ""){
      ...
      for(var i = 0; i < arr.length; i++){
        ...
        this.result[groupI].items.push({"name": arr[i].querySelector('p').innerText});
      }
    }
  }
  ...
  getKeyCode(e:any){ e.code === 'Enter' ? this.itemAdded() : '' }

  // Add New Group on Enter
  addGroupKeyCode(e:any){ e.code === 'Enter' ? this.addGroup(): '' }

  // Add New Item on Enter
  addNewItemField(e:any, i:number){ e.code === 'Enter' ? this.pushNamedItem(i) : '' }
